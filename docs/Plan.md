...
**1.1. Успешная покупка при оплате картой с валидным номером и статусом APPROVED**
...
**1.2. Отказ в покупке при оплате картой с валидным номером и статусом DECLINE4444444444444442D**
1. Заполнить все поля валидными данными карты со статусом DECLINED (Номер карты - , Месяц - 10, Год - 26, Владелец - Ivan Ivanov, CVC/CVV - 123)
2. Нажать на кнопку "Продолжить"
...
**2.1. Успешная покупка в кредит при оплате картой с валидным номером и статусом APPROVED**
1. Заполнить все поля валидными данными карты со статусом APPROVED (Номер карты - 4444444444444441, Месяц - 10, Год - 26, Владелец - Ivan Ivanov, CVC/CVV - 123)
2. Нажать на кнопку "Продолжить"
   *Ожидаемый результат: В правом верхнем углу экрана появляется сообщение "Успешно Операция одобрена банком"*
...
Данные для карты со статусом APPROVED: Номер карты - 4444444444444441, Месяц - 10, Год - 26, Владелец - Ivan Ivanov, CVC/CVV - 123
Данные для карты со статусом DECLINED: Номер карты - 4444444444444442, Месяц - 10, Год - 26, Владелец - Ivan Ivanov, CVC/CVV - 123
...
1. Java 11 - универсальный язык, позволяющий работать на всех ОС и различном оборудовании за счет виртуальной машины, которая полностью контролирует безопасность;
...
7. Lombok - библиотека для сокращения кода в классах и расширения функциональности языка Java, освобождает от необходимости сручного написания конструкторов, геттеров, сеттеров и тд.;
...
- Недоступность css-селекторов для проверки вследствие изменения html, css структуры веб-сервиса
...
- Подготовка отчёта по автоматизации - 2 рабочих для
